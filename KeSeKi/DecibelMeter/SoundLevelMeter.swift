...
final class SoundLevelMeter: DecibelMeasuring {
    ...
    func start(onLevel: @escaping (Float) -> Void,
        ...
        Self.requestMicPermission { granted in
            ...
            do {
                try session.setCategory(.record, options: [.allowBluetoothHFP, .mixWithOthers])
            ...
            self.timer = Timer.scheduledTimer(withTimeInterval: 0.05, repeats: true) { [weak self] _ in
                ...
                rec.updateMeters()
                onLevel(rec.averagePower(forChannel: 0)) // -60 ~ 0 dBFS
            }
        }
    }
